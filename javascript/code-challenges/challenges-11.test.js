...
function transformToLis(obj){
  // Convert obj into array of key value pairs (['name', 'bob']), map through array of key value pairs, for each key value pair return html <li> elements
...
const count = (target, input) => {
  ...
  return input.reduce((acc, array) => // Return acc value holding how many times target appears in array
    acc + array.filter(num => // Check to see if any num is equal to target
      num === target).length, 0); // If num equal to target, store into array, length of array representing how many times number appeared in array
...
const totalSum = (input) => {
  ...
  return input.reduce((acc, array) => // Return acc value (total sum of adding each number in each array)
    acc + array.reduce((sum, element) => // Add total of sum of each subarray to acc
      sum + element, 0), 0); // Add each element to sum (running total)
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(subArray => // Map each sub-array
    subArray.filter(value => typeof value === 'number' && value % 5 === 0) // Filter values
      .map(num => Math.pow(2, num)) // Raise 2 to the power of the number
  );
...
let findMaleAndFemale = (data) => {
  ...
  return data
    .filter(character => character.gender === 'male' || character.gender === 'female') // Filter by male or female
    .map(character => character.name) // Map array of character names
...
let findShortest = (data) => {
  ...
  // Convert character height from string to num, compare all elements, return name of shortest character.
  return data.reduce((shortest, character) =>
    (parseInt(character.height, 10) < parseInt(shortest.height, 10) ? character : shortest) // Compare each element to see if it is 'shortest' value
  ).name; // No need for acc initial value. If no intiial acc provided, first element of array is default intial value
