...
const sortByChildren = (charArray) => {
  ...
  return charArray.sort( (characterA, characterB) => {

    if ( characterA.children < characterB.children) {
      return -1; // a comes before b

    } else if (characterA.children > characterB.children) {
      return 1; // b comes before a

    } else {

      if (characterA.name < characterB.name) { // sort by name
        return -1; // a comes before b
...
const containsW = (str) => {
  ...
  let regex = /w/g; // Find matches for any word that has 'w' in lower case
...
const isCapitalized = (str) => {
  ...
  return matches ? matches : []; // return array of matches, else return empty array
};
