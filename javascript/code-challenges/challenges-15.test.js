...
const toTitleCase = (arr) => {
  // Uppercase position 0 of string, and add to sliced string starting at position 1 (second character)
...
let biggerThanLuke = (arr) => {
  ...
  // Check if Luke found, if not found return empty string
  if (!luke) {
    return '';
  }

  // Retrieve Luke's mass
  const lukeMass = parseInt(luke.mass, 10);

  // Filter out character with mass greater than Lukes, crate array of only character names, join character names with ' - '
  return arr
    .filter ( character => parseInt(character.mass, 10) > lukeMass)
...
const sortBy = (property, arr) => {
  ...
  // Use bracket notation instead of dot notation to access objects, as property variable is dynamic.  Use dot notation when know exact property name want to use.
  return arr.sort( (a, b) => {
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  const regex = /^(https:\/\/)+(www)?[\w]+\.(com|org)$/;
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < 3; i++) {
    if (helpCheck(board, i, 0, i, 1, i, 2) || helpCheck(board, 0, i, 1, i, 2, i)) {
      return true;
    }
  }
