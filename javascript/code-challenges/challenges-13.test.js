...
const longestString = (arr) => {
  ...
  // Return longest string index
  return arr.reduce( (longestIndex, currentString, currentIndex) => {
    return currentString.length > arr[longestIndex].length ? currentIndex : longestIndex;
  }, 0); // Start with 0 as initial longest
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(string => string.replace(/[() -]/g, '') );
};
...
const onlyOddChars = (str) => {
  return [...str] // Spread string into an array of characters
    .filter( (char, index) => index % 2 !==0) // Filter out even-index characters
...
const unenrollBrook = (arr) => {
  return arr.map(courseRoster =>
    courseRoster.filter(student => !student.includes('Brook'))
  );
};
...
const sortByDay = (arr) => {
  let week = daysOfWeek.map(() => []); // Initialize array for each day of the week
...
const characterByIndex = (arr) => {
  // For each string check if length is greater than its index in array. If so, return character at that index. If not, return null
  return arr.map( (string, index) => string.length > index ? string[index] : null);
};
